...
class BotWrapper:
    ...
    def catch_exeptions(self, update, context):
        context.bot.send_message(update.message.chat.id, self.logger.warning('Update "%s" caused error "%s"', update, context.error))
    ...
    def echo(self, update, context):
        ...
        if not user_id in id:
            self.connection.execute(self.statistics.insert(), {"user_id": user_id, 'right_answers': 0, 'wrong_answers': 0, 'stamp': ''})
    ...
    def create_reply_button(self):
        ...
        self.reply_keyboard = ReplyKeyboardMarkup([[button]], one_time_keyboard=False)

    #def zero_stastistics(self, update, context):
